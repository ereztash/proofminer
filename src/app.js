...
function allocationSegmentsHTML(){
  ...
  const used=state.actions.filter(a=>a.hours>0).map(a=>{
    ...
    return `<div class="resource-segment" style="--segment:${w}%" title="${esc(a.title)} · ${esc(a.hours)} שעות"><span>${esc(a.hours)}h</span></div>`;
  }).join('');
...
function challengeBlock(a,c){const changed=c.suggestedStatus!==a.status;return `<div class="challenge-block ${c.applied?'accepted':''} ${c.rejected?'rejected':''}"><div class="challenge-head"><span>אתגר של המערכת</span>${changed?`<b>${statusLabel(a.status)} → ${statusLabel(c.suggestedStatus)}</b>`:'<b>בדיקת הנחה</b>'}</div><p>${esc(c.reason)}</p><details><summary>מה יגרום לאתגר הזה להיעלם?</summary><p>${esc(c.reverse)}</p></details><div class="challenge-actions"><button class="mini-primary" data-apply-suggestion="${esc(a.id)}">החל את השינוי</button><button class="mini-secondary" data-reject-suggestion="${esc(a.id)}">אני לא מקבל</button></div></div>`;}
